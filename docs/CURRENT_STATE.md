...
**Current status:** K-1 through K9 are complete and v0.58/schema v40 is merged and live. Kevin confirmed Groq ZDR, Mistral Free Mode, and Gemini free-tier data use. Groq passed on `openai/gpt-oss-20b`. A focused Gemini repair replaced the unavailable new-user `gemini-2.5-flash` target with `gemini-flash-latest`, moved every relay route to Google's `x-goog-api-key` header, and supplied the strict response JSON schema. The authenticated synthetic preview resolved to `gemini-3.6-flash` and passed schema/privacy/proposal-only checks in 3,743 ms with no retained content. Groq remains the sole production-active route until this focused change is merged and deployed; Mistral, Workers AI, and optional routes remain disabled and `AI_ALLOW_PAID=false`.
...
- Groq is the sole production-enabled fabric route at exact pair `groq:openai/gpt-oss-20b`; its ZDR confirmation, strict receipt, usage, and live free-plan rate headers are recorded. Gemini is a verified activation candidate through the `gemini-flash-latest` alias, resolving to exact model `gemini-3.6-flash`; production activation awaits this branch's merge/deploy. Mistral is configured/Free-Mode-confirmed but disabled after `OUTPUT_SCHEMA`; optional providers remain disabled. Workers AI is binding/model reachable but disabled after `OUTPUT_SCHEMA`. `AI_ALLOW_PAID=false` remains sealed in the Worker.
