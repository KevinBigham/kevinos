...
Attention evidence is an explicitly enabled device-local recorder. Its allowlisted receipts are bounded and sanitized, are restored on local boot, and are deliberately absent from portable documents, sync documents, relay requests, and AI context. Pure digest/signal functions provide at most one calm evidence-backed intervention without participating in `nowModel()`.
