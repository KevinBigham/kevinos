...
- **App:** `v0.15`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.15, ~2050 lines, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_15", + push / notificationclick)
...
- **Phase 2b (second half) — GITHUB OAUTH (token off-device):** the relay gained `/github/login`, `/github/callback` (code→token, stored in KV), `/github/status`, `/github/graphql` (proxy with the server-side token), `/github/logout` (revoke). The GitHub room now offers a one-tap **Connect with GitHub** (OAuth) that polls until connected and proxies all GitHub data through the relay; the PAT path stays as an "Advanced" fallback. `state.v` → 15 — v0.15. **Activates once Kevin registers the OAuth App** (`GITHUB_CLIENT_ID` + secret).

**Next, when Kevin says go (do NOT start unprompted):**
- **Phase 2b — DONE (pending activation):** Web Push (v0.14) + GitHub OAuth (v0.15) are both shipped. GitHub OAuth **activates the moment Kevin registers the OAuth App** — add `GITHUB_CLIENT_ID` to `wrangler.toml`, `npx wrangler secret put GITHUB_CLIENT_SECRET`, redeploy. Callback to register: `https://kevinos-relay.kevinbigham.workers.dev/github/callback`, scope `read:user repo`. Optional later: an email-to-self backstop.
- **Phase 3 — Sync (next):** Supabase (free tier), last-write-wins + `updatedAt`, one dataset across Mac + phone.
