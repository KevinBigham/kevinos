...
- **App:** `v0.19`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.19, ~2560 lines, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_19", + push / notificationclick)
...
- **Email Command Center (v0.18):** `/google/login` → consent → `/google/callback` (multi-account; stores **refreshable** Gmail tokens in the `PUSH` KV under `gml:<session>` → `{accounts:[{email,access,refresh,exp}]}`); `/google/status` (poll); `/google/threads` (list INBOX); `/google/draft` (Gemini drafts a reply); `/google/send` (sends via **gmail.send** with In-Reply-To/References/threadId); `/google/logout` (revoke). The browser holds only a session id + account emails — **never a token**. var `GOOGLE_CLIENT_ID` + secret `GOOGLE_CLIENT_SECRET` (empty until Kevin registers → routes degrade gracefully). `GET /` health gained `email:bool`.

**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract, email }` (`push` = VAPID configured; `github`/`email` = OAuth configured; `sync` = D1 bound; `extract` = Gemini key present; seats = currently-live roster)
...
- **Cross-device sync (v0.16):**
  ...
  - `POST /sync/push` `{key, doc, updatedAt, deviceId, rev?}` → upserts the document; **last-write-wins** — a strictly-newer stored version is never clobbered (returns `{ok:false, stale:true, doc, updatedAt, rev}` so the app reconciles), otherwise `{ok:true, rev, updatedAt}`. `key` must be 16–128 hex chars (the `sha256(passphrase)` the app sends).
...
- **Beyond (none started — do NOT start unprompted):** the **smart AI morning brief** (upgrade the 8am push to an AI cockpit summary); overnight AI email drafting (cron); unified multi-account inbox triage; Outlook.
