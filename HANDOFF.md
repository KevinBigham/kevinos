...
- **Web Push (v0.14):** secret `VAPID_PRIVATE_KEY` (ES256 signing key — set via `wrangler secret put`, lives only on Cloudflare); vars `VAPID_PUBLIC_KEY` (public, advertised to the app as `applicationServerKey`) + `VAPID_SUBJECT`. Binding `PUSH` (KV namespace, id in `wrangler.toml`). Cron `crons = ["*/2 * * * *"]` runs every 2 minutes. The encryption (`encryptPayload`) is RFC 8291 `aes128gcm` done entirely in WebCrypto — **no `web-push` dependency** — and was verified byte-for-byte against the RFC 8291 test vector before shipping.
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract, capture, summarize, spend, launch, calendar, habits, email, peopleEnrich }` (`push` = VAPID configured; `github`/`calendar`/`email`/`peopleEnrich` = OAuth configured; `sync`/`habits` = D1 bound; `extract`/`capture`/`summarize`/`spend`/`launch` = Gemini key present; seats = currently-live roster)
...
- **Web Push (v0.14):**
  ...
  - **cron** `*/2 * * * *` → `scheduled()` → `firePush(env)`: runs every 2 minutes and fires every reminder whose `fireAt ≤ now` (1h stale-grace), then drops it; a 404/410 from the push service deletes the subscription. The app owns recurrence by re-syncing the next occurrence.
- **GitHub OAuth (v0.15):**
...
- **Phase 2b (first half) — PHONE REMINDERS (Web Push):** the relay gained VAPID + RFC-8291 `aes128gcm` encryption (WebCrypto, no library), a `PUSH` KV store, and a two-minute cron; the app subscribes via `pushManager` and syncs its reminder set (`/push/sync`). Two reminder types — a **morning brief** (chosen hour) + **per-task due-time** nudges. Tasks gained an optional due time; `state.v` → 14 — v0.14.
