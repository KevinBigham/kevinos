...
- **App:** `v0.35`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.35, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_35", + push / notificationclick)
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract, capture, summarize, spend, calendar, habits, email, peopleEnrich }` (`push` = VAPID configured; `github`/`calendar`/`email`/`peopleEnrich` = OAuth configured; `sync`/`habits` = D1 bound; `extract`/`capture`/`summarize`/`spend` = Gemini key present; seats = currently-live roster)
...
- **Next mission feature:** #10 Morning Launch Sequence.
