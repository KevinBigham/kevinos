...
- **App:** `v0.16`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.16, ~2200 lines, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_16", + push / notificationclick)
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync }` (`push` = VAPID configured; `github` = OAuth configured; `sync` = D1 bound; seats = currently-live roster)
