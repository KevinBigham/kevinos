...
- **App:** `v0.33`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.33, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_33", + push / notificationclick)
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract, capture, summarize, calendar, habits, email, peopleEnrich }` (`push` = VAPID configured; `github`/`calendar`/`email`/`peopleEnrich` = OAuth configured; `sync`/`habits` = D1 bound; `extract`/`capture`/`summarize` = Gemini key present; seats = currently-live roster)
...
- **Next mission feature:** #7 People Radar, then #8 Spend Pulse, #9 Goals & Weekly Check-In, #10 Morning Launch Sequence.
