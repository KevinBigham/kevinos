...
- **App:** `v0.17`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.17, ~2350 lines, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_17", + push / notificationclick)
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract }` (`push` = VAPID configured; `github` = OAuth configured; `sync` = D1 bound; `extract` = Gemini key present; seats = currently-live roster)
...
- **Phase 5 — Email Command Center (NEXT, built last):** **multi-account Gmail** via relay OAuth, AI drafts replies → review queue → approve → **sent on approval** (`gmail.send` — Kevin chose full one-tap send; still never auto-sends without a human approve). Needs Kevin's one-time **Google Cloud OAuth** registration (Web app; redirect `https://kevinos-relay.kevinbigham.workers.dev/google/callback`; scopes `gmail.readonly gmail.send` + `userinfo.email`; keep the app in **Testing** mode and add his Gmail accounts as test users to dodge the CASA audit). Outlook can be added later (Kevin chose Gmail-first).
