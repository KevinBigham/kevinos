...
- **App:** `v0.30`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract, capture, calendar, habits, email }` (`push` = VAPID configured; `github`/`calendar`/`email` = OAuth configured; `sync`/`habits` = D1 bound; `extract`/`capture` = Gemini key present; seats = currently-live roster)
...
- **Next mission feature:** #4 One-Tap Send, then #6 Link Stash, #7 People Radar, #8 Spend Pulse, #9 Goals & Weekly Check-In, #10 Morning Launch Sequence.
