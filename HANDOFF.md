...
- **App:** `v0.32`, live at **https://kevinbigham.github.io/kevinos/** (GitHub Pages, public repo `KevinBigham/kevinos`).
...
│   ├── index.html                   ← THE APP (v0.32, ES5)
│   ├── manifest.json                ← PWA manifest (+ share_target)
│   ├── sw.js                        ← service worker (CACHE = "kevinos-v0_32", + push / notificationclick)
...
**Endpoints:**
- `GET /` → health `{ ok, service, provider, seats:[…], push, github, sync, extract, capture, summarize, calendar, habits, email }` (`push` = VAPID configured; `github`/`calendar`/`email` = OAuth configured; `sync`/`habits` = D1 bound; `extract`/`capture`/`summarize` = Gemini key present; seats = currently-live roster)
